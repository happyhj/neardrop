...
*/
Neighbor.prototype.getUserImageFileName = function(id) {
	return "user-"+(this.getIndexFromDigit(id.charAt(id.length-4))+1)+".jpg";
}



Neighbor.prototype.bump = function() {
	this.spring.setCurrentValue(2.3);
	this.spring.setEndValue(2);	
}


Neighbor.prototype.listenShout = function() {
	if(this.shoutDelegate === undefined)
		this.shoutDelegate = new ShoutDelegate(this.id);
	else {
		if(this.shoutDelegate.connection && this.shoutDelegate.connection.open === true) {
			this.shoutDelegate.connection.send({
				  "kind" : "shouting",
				  "id" : app.airdrop.peer.id
			});				
		} else {
			//this.shoutDelegate.connection = this.shoutDelegate.peer.connect(this.shoutDelegate.targetPeerId);		
		}
	}	
}
/*
...
*/			
Neighbor.prototype.getUserName = function(id) {
	var gender = this.getIndexFromDigit(id.charAt(id.length-2))%2;
	gender = (gender===0)?"Boys":"Girls";
	
	var nameIdx = this.getIndexFromDigit(id.charAt(id.length-3))%35;
	var name = this.nameDictionary[gender][nameIdx];
