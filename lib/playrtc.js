...
var Logger = (function(){
	...
	(function(_){
		...
		_.prepareSend = function(){
			this.req.open("post", this.logger.playRtc.config.url + "/v1/playrtc/mnitoring/log", true);
...
var NAGSignalManager = utils.Extend(utils.Event, {
	...
	userRegistration: function(){
		...
		url = this.call.playRtc.config.url + "/v1/playrtc/nag/authentication/user/" + this.call.getPid().substr(2);
	...
	userDeletion: function(){
		...
		var url = this.call.playRtc.config.url + "/v1/playrtc/nag/authentication/user/" + this.call.getPid().substr(2);
		this.requestAjax(
			"delete", url,
			"application/json; charset=UTF-8",
			"{}", success, error
...
var NAGSignalingAdapter = PlayRTC.Signaling.Extend({
	...
	parseSdp: function(sdp){
		...
		for(; i<sdp.length; i++){
			...
			if(/^audio/.test(sdp[i])){
				j = 0;
				result = result + sdp[i];
				for(; j<this.sdpMidAudio.length; j++){
					if(/^a=/.test(this.sdpMidAudio[j].candidate)){
						result = result + this.sdpMidAudio[j].candidate;
					}
					else{
						result = result + "a=" + this.sdpMidAudio[j].candidate + "\r\n";
					}
				}
			}
			else if(/^video/.test(sdp[i])){
				j = 0;
				result = result + sdp[i];
				for(; j<this.sdpMidVideo.length; j++){
					if(/^a=/.test(this.sdpMidVideo[j].candidate)){
						result = result + this.sdpMidVideo[j].candidate;
					}
					else{
						result = result + "a=" + this.sdpMidVideo[j].candidate + "\r\n";
					}
					
				}
			}
			else if(/^application/.test(sdp[i])){
				j = 0;
				result = result + sdp[i];
				for(; j<this.sdpMidData.length; j++){
					if(/^a=/.test(this.sdpMidData[j].candidate)){
						result = result + this.sdpMidData[j].candidate;
					}
					else{
						result = result + "a=" + this.sdpMidData[j].candidate + "\r\n";
					}
				}
			}
		}
	...
	sendCandidate: function(id, candidate){
		if(candidate.sdpMid === "audio" || candidate.sdpMLineIndex === 0){
			this.sdpMidAudio.push(candidate);
		}
		else if(candidate.sdpMid === "video" || candidate.sdpMLineIndex === 1){
			this.sdpMidVideo.push(candidate);
		}
		else if(candidate.sdpMid === "data" || candidate.sdpMLineIndex === 2){
			this.sdpMidData.push(candidate);
		}
	},
	sendOfferSdp: function(id, sdp){
		var splitSdp = null,
...
(function(){
	...
	PlayRTC.Helper = {
		...
		createChannel: function(options, success, error){
			...
			var url = this.url + "/v1/playrtc/channels/channel-extend/channelId";
			requestAjax(
				url,
				"post",
				callback,
		...
		connectChannel: function(channelId, options, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/channel-extend/{0}/tokens/token", channelId);
			requestAjax(
				url,
				"post",
				callback,
		...
		disconnectChannel: function(channelId, peerId, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/channel-extend/{0}/peers/{1}", channelId, peerId);
			requestAjax(
				url,
				"delete",
				callback
		...
		deleteChannel: function(channelId, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/channel-extend/{0}", channelId);
			requestAjax(
				url,
				"delete",
				callback
		...
		getChannelList: function(success, error){
			...
			var url = this.url + "/v1/playrtc/channels";
			requestAjax(
				url,
				"get",
				callback
		...
		getChannel: function(channelId, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/{0}/info", channelId);
			requestAjax(
				url,
				"get",
				callback
		...
		getPeerList: function(channelId, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/{0}/peers", channelId);
			requestAjax(
				url,
				"get",
				callback
		...
		getPeer: function(channelId, peerId, success, error){
			...
			var url = this.url + utils.strFormat("/v1/playrtc/channels/{0}/peers/{1}", channelId, peerId);
			requestAjax(
				url,
				"get",
				callback
